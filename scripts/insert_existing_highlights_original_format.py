...
OUTPUT = r"C:\Users\weimo\Downloads\具身智能社团官网——代码架构与设计 (3)-按原格式补充版.docx"
...
content = [
    ...
    ("代码解释：app.js 中的 newsCache 保存 payload 和 loadedAt，60 秒内直接返回缓存；newsRequest 保存当前请求，存在时直接返回该 Promise。请求结束后在 finally 中清空 newsRequest，保证下一次失效读取可以重新发起。", "body"),
    ("设计要点：", "body"),
    ...
    ("代码解释：src/main.jsx 使用 oculotronics:scroll:<page> 作为存储键，在 pagehide 和 effect 清理时保存 window.scrollY。恢复时连续等待两次 requestAnimationFrame；新闻数据加载完成后，app.js 派发 oculotronics:content-ready，壳层收到后再次恢复位置。", "body"),
    ("设计要点：", "body"),
    ...
    ("代码解释：app.js 创建 IntersectionObserver 观察 reveal、feature-row 和 section-reveal 元素；滚动进度和部分高频视觉变化通过 requestAnimationFrame 合并到下一帧。代码在 cleanup 中调用 observer.disconnect() 和 splitTextObserver.disconnect()。", "body"),
    ("设计要点：", "body"),
    ...
    ("代码解释：src/pot-player.js 在切换清晰度时记录 currentTime，监听 loadedmetadata 后恢复进度；translationCache 以 targetLanguage:sourceText 为键，并由 MAX_TRANSLATION_CACHE_ENTRIES=300 控制最大数量，防止长时间使用播放器造成内存无限增长。", "body"),
    ("设计要点：", "body"),
